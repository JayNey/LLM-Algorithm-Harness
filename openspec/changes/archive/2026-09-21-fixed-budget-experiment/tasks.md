...
- [x] 聚合各组合结果：隐藏通过率、样例验证率、失败类型分布（含 budget_exhausted）、修复率、平均调用数 / token / 耗时；按难度与标签分组
- [x] 重复 ≥ 2 时输出跨重复最小 / 最大范围与不确定性说明
- [x] 输出 `comparison.json` / `comparison.csv` / `REPORT.md`；成本未知显示"未知"
- [x] 测试：固定夹具（正确解 / 样例过拟合 / API 失败 / 预算耗尽）手算对账，分母 = 成功 + 各失败 + 未完成
